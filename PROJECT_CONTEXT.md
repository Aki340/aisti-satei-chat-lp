# PROJECT_CONTEXT.md — aisti-satei-chat-lp（汎用版）

## プロジェクト概要

本リポジトリは、不動産査定チャット型ランディングページ（LP）の**汎用版**です。ユーザーがチャット形式で不動産査定に必要な情報を入力し、査定依頼へ誘導するコンバージョン最適化型のLPを提供します。地域固有のカスタマイズを行わない汎用テンプレートとして機能し、大阪版・福岡版・東京版の各地域リポジトリのベースとなっています。

## 技術スタック

| 項目 | 内容 |
|------|------|
| フロントエンド | バニラJS（Vanilla JavaScript） |
| ホスティング | GitHub Pages |
| サーバーレス関数 | Cloudflare Workers |
| バージョン管理 | GitHub |

## リポジトリの役割

- **種別**: 汎用版（地域非依存のベーステンプレート）
- **用途**: 地域を問わない不動産査定チャット型LPの提供
- **質問ステップ数**: 8ステップ

## 最新安定タグ

```
v1.2.6
```

## 公開URL

https://tnkkakuei3-design.github.io/aisti-satei-chat-lp/

## 関連リポジトリ

| リポジトリ | 種別 | 最新タグ | 公開URL |
|-----------|------|---------|--------|
| aisti-satei-chat-lp | 汎用版 | v1.2.6 | https://tnkkakuei3-design.github.io/aisti-satei-chat-lp/ |
| aisti-lp-osaka | 大阪版 | aisti-lp-v1.2.7 | https://tnkkakuei3-design.github.io/aisti-lp-osaka/ |
| aisti-lp-fukuoka | 福岡版 | aisti-lp-fukuoka-v1.0.8 | https://tnkkakuei3-design.github.io/aisti-lp-fukuoka/ |
| aisti-lp-tokyo | 東京版 | aisti-lp-tokyo-v1.0.8 | https://tnkkakuei3-design.github.io/aisti-lp-tokyo/ |

## 更新履歴

| 日付 | バージョン | 内容 |
|------|-----------|------|
| 2026-03-10 | v1.2.6 | 最新安定版 |
